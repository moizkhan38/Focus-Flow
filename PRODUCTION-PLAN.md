...
### Step 3.6 — Deploy + production smoke `[x]` **USER-ACTION (accounts/DNS) + Opus**
**Size:** M
...
| 2026-07-22 | 3.6 | (deployed to Vercel + Railway; live at focusflowpk.com) | **PRODUCTION DEPLOY — the app is LIVE and AI generation works end to end.** Frontend on **Vercel** (custom domain `focusflowpk.com` via Cloudflare DNS, grey-cloud/DNS-only CNAMEs). Backend + Flask + Postgres on **Railway** (Managed-PaaS path; Cloud Run's 5 fixes not needed since Railway runs the containers persistently). Live health verified: frontend 200, backend `/api/ready {db:true}`, Flask Gemini available, generation confirmed by user. **Issues hit + fixed during deploy (all config, no code):** (1) Vercel 404 — monorepo Root Directory unset + Framework Preset "Other" → set root `epic-dev-assignment/frontend` + preset Vite + `vercel.json` SPA rewrite; (2) env var names — Clerk's Next.js snippet (`NEXT_PUBLIC_*`) vs this Vite app's `VITE_*`; `CLERK_SECRET_KEY` wrongly on the frontend → removed; (3) `VITE_API_URL` missing at project level (was only team-Shared) → added, rebuilt; (4) Cloudflare "Invalid Configuration" — orange-cloud proxy → switched to DNS-only; (5) repo renamed `→ focus-flow` (only the Railway *service* needed renaming) → local remote updated, GitHub redirects held; (6) Railway monorepo Root Directory per service (`epic-dev-assignment/backend`, `epic-generator`); (7) **the "failed to fetch" on generate = `INTERNAL_API_KEY` mismatch between backend and Flask** — Flask HTTP logs showed `POST /api/generate → 401`, backend wrapped it as 500; fixed by matching the keys. Secrets: fresh `CREDENTIALS_MASTER_KEY` + `INTERNAL_API_KEY` generated for prod (local key stays local); GitHub PAT already rotated at G2. **Not yet done for a hardened public launch (G3):** Clerk still on the **dev** instance (`pk_test`) — production instance + domain wiring needed before real users; Gemini billing (works now on free-tier `gemini-flash-lite-latest`, enable before load); legal pages (privacy/ToS) before non-self customers; Sentry; uptime monitor on `/api/ready`; managed-PG backups verification; the standup bot is not deployed (optional, D6). |
